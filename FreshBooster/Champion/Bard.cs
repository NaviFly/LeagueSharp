...
namespace FreshBooster.Champion
{
    class Bard
    {
        ...
        public static Obj_AI_Hero RRange;
            ...
            try
            {
                ...
                if (_MainMenu.Item("CKey").GetValue<KeyBind>().Active)
                {
                    if (_MainMenu.Item("Bard_CUse_Q").GetValue<bool>() && _Q.IsReady() && QTarget != null)
                    {
                        ...
                        {
                            if (cnt == 2)
                                if (BardQTarget1 is Obj_AI_Hero || BardQTarget2 is Obj_AI_Hero)
                                    _Q.CastIfHitchanceEquals(BardQTarget1, Hitchance("Bard_CUse_Q_Hit"), true);
                            if (cnt == 1 && BardQTarget1 is Obj_AI_Hero && BardQTarget1.Position.Extend(Player.Position, -450).IsWall())
                                _Q.CastIfHitchanceEquals(BardQTarget1, Hitchance("Bard_CUse_Q_Hit"), true);
                        }
                        else
                        {
                    ...
                    if (_MainMenu.Item("Bard_HUse_Q").GetValue<bool>() && _Q.IsReady() && QTarget != null && _MainMenu.Item("Bard_AManarate").GetValue<Slider>().Value < Player.ManaPercent)
                    {
                        ...
                        if (_MainMenu.Item("Bard_HUse_OnlyQ").GetValue<bool>())
                        {
                            if (cnt == 2 && Range2 != null)
                                if ((BardQTarget1 is Obj_AI_Hero && BardQTarget1 != Player) || (BardQTarget2 is Obj_AI_Hero && BardQTarget2 != Player))
                                {
                                    _Q.CastIfHitchanceEquals(BardQTarget1, Hitchance("Bard_CUse_Q_Hit"), true);
                                    Console.WriteLine("1");
                                }

                            if (cnt == 1 && BardQTarget1 is Obj_AI_Hero && BardQTarget1.Position.Extend(Player.Position, -400).IsWall())
                            {
                                _Q.CastIfHitchanceEquals(BardQTarget1, Hitchance("Bard_CUse_Q_Hit"), true);
                                Console.WriteLine("2");
                            }
