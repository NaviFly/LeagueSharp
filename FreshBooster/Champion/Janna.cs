    ...
    class Janna
    {
        ...
        private static void OnGameUpdate(EventArgs args)
        {
            try
            {
                ...
                {
                    var AutoR = ObjectManager.Get<Obj_AI_Hero>().OrderByDescending(x => x.Health).FirstOrDefault(x => x.HealthPercent < _MainMenu.Item("Janna_AutoRHP").GetValue<Slider>().Value && x.Distance(Player.ServerPosition) < _R.Range && !x.IsDead && x.IsAlly);
